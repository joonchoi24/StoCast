...
X_non = grad.drop(columns=['Next_Close','Positive_Spike', 'Long_Change','SP_Change','Gap']) 
...
start_date, end_date = st.date_input('Select date range:', [grad.index.min(), grad.index.max()], min_value=grad.min(axis=0), max_value=grad.max(axis=0))
